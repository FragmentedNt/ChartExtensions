...
namespace MathRoundExtension
{
	static class MathEx
	{
		...
		public static double Round(double val, int digit, MidpointRounding midpointRounding = MidpointRounding.AwayFromZero)
		{
		...
		{
			return ((int)Math.Ceiling(Normalize(val) * Math.Pow(10, digit))) / 10;
		}
		...
		{
			return new String(Normalize(val).ToString()
								 .Where((c, i) => (c != '0' || i != 0) && c != '.')
								 .ToArray())
